#!/usr/bin/env python3
"""OpenAI responder daemon for research and coding tasks via hot vessel API."""
...
from openai_resident import ResidentOpenAI # Import the local ResidentOpenAI class
...
class OpenAIResponder:
    """Processes COMMS_HUB messages directed at the OpenAI vessel."""

    def __init__(self) -> None:
        self.events = TrinityEventStream()
        # Initialize ResidentOpenAI directly
        self.resident = ResidentOpenAI()
        self.comms = CommsHubClient("openai", event_stream=self.events)
        self.running = True
    ...
    def _handle_query(self, message: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        extra = {k: v for k, v in payload.items() if k != "query"}
        response_data = self.resident.route_and_respond("comms_hub_query", query)
        response_text = response_data.get("answer", "OpenAI resident returned no content.")
        
        # Log event for monitoring
        self.events.log_event(
            source="openai_responder",
    ...
    def _handle_task(self, message: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        _send_message("openai", recipient, "task_complete", response_payload, priority)
    ...
    def run(self) -> None:
        self.events.log_event(source="openai_responder", event_type="started", data={})
        while self.running:
            try:
        ...
        self.events.log_event(source="openai_responder", event_type="stopped", data={})
