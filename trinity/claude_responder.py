#!/usr/bin/env python3
"""Claude responder daemon wired into the COMMS_HUB using the hot vessel API."""
...
from claude_resident import ResidentClaude # Import the local ResidentClaude class
...
class ClaudeResponder:
    """Simple polling loop that processes COMMS_HUB messages for Claude."""

    def __init__(self) -> None:
        self.events = TrinityEventStream()
        # Initialize ResidentClaude directly
        self.resident = ResidentClaude()
        self.comms = CommsHubClient("claude", event_stream=self.events)
        self.running = True
    ...
    def _handle_query(self, message: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        response_text = response_data.get("answer", "Claude resident returned no content.")
        
        # Log event for monitoring
        self.events.log_event(
            source="claude_responder",
    ...
    def _handle_task(self, message: Dict[str, Any], payload: Dict[str, Any]) -> None:
        ...
        _send_message("claude", recipient, "task_complete", response_payload, priority)
    ...
    def run(self) -> None:
        self.events.log_event(source="claude_responder", event_type="started", data={})
        while self.running:
            try:
        ...
        self.events.log_event(source="claude_responder", event_type="stopped", data={})
